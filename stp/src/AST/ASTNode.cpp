...
namespace BEEV 
{
  ...
  CBV ASTNode::GetBVConst() const
  {
    ...
    return ((ASTBVConst *) get())->GetBVConst();
  } //End of GetBVConst()
            ...
            if (bm->NodeLetVarMap.find(ccc) == bm->NodeLetVarMap.end())
              {
                ...
                ASTNode CurrentSymbol = bm->CreateSymbol(oss.str().c_str(),GetIndexWidth(),GetValueWidth());
